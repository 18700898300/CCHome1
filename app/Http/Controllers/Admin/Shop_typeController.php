...
class Shop_typeController extends Controller
{
    ...
    */
    public function add()
    {
        ...
        return view('admin/shop_type/add',compact('cateOne'));
    }
    /*
     * 执行添加操作
     */
...
//        2. 表单验证
       $rule = [
        ...
        //如果表单验证失败 passes()
        if ($validator->fails()) {
            return redirect('admin/shop_type/add')
...
//        4. 判断是否成功
        if($res){
            return redirect('admin/shop_type/lists')->with('msg','添加成功');
        }else{
...
//        获取所有的分类列表
        ...
        //dd($cates);
        return view('admin/shop_type/lists',compact('cates'));
    }
...
//        dd($cate);
//        3. 携带信息引入视图
        return view('admin/shop_type/edit',compact('cateOne','cate'));
    }
...
//        2. 表单验证
        ...
        //如果表单验证失败 passes()
        if ($validator->fails()) {
            return redirect('admin/shop_type/edit/'.$input['tid'])
                ->withErrors($validator)
                ->withInput();
        }
//        3. 执行修改
...
//        4. 判断是否成功
        if ($res) {
            return redirect('admin/shop_type/lists')->with('msg','修改成功');
        } else {
            return redirect('admin/shop_type/edit/'.$input['tid'])->with('msg','修改失败');
        }
