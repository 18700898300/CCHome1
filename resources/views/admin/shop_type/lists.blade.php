...
@section('body')
            ...
            <div class="result_content">
                ...
                @foreach($cates as $k=>$v)
                    <tr>
                        ...
                        <td>
                            <a href="{{url('admin/shop_type/edit')}}/{{$v->tid}}">修改</a>
    ...
    <script>
        ...
        function changeOrder(obj,tid){
            ...
            //获取当前记录的排序文本框中的值
            var torder = $(obj).val();
            $.post("{{url('admin/shop_type/changeorder')}}",{'_token':"{{csrf_token()}}","tid":tid,"torder":torder},function(data){
                //如果排序成功，提示排序成功
                if(data.status == 0){
...
//                $.get("请求服务器的路径","携带的参数", 获取执行成功后的额返回数据);
                //admin/user/1
                $.post("{{url('admin/shop_type/delCate')}}/"+id,{"_token":"{{csrf_token()}}"},function(data){
                    //alert(data);
